...
export function migrateSchema(db: Database): void {
  ...
  // Remove old configs that have been replaced
  db.run(`DELETE FROM research_configs WHERE id = 'markets-portfolio'`);

  // SQLite CHECK constraints can't be altered, so we need to recreate the table
  // to allow the 'politics' category. Check if politics is already allowed by trying an insert.
  ...
  if (!existingPolitics) {
    try {
      ...
      // CHECK constraint might block this on old databases - need to recreate table
      console.log('Politics insert failed (likely old CHECK constraint), recreating table...');

      // Backup existing data
      const existingConfigs = db.query(`
        SELECT id, name, description, prompt, category, topics, preferred_sources, blocked_sources, enabled, schedule
        FROM research_configs
      `).all() as any[];

      // Drop and recreate with new constraint
      db.run('DROP TABLE research_configs');
      db.run(`
      ...
      const insertStmt = db.prepare(`
        INSERT INTO research_configs (id, name, description, prompt, category, topics, preferred_sources, blocked_sources, enabled, schedule)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      for (const cfg of existingConfigs) {
        insertStmt.run(cfg.id, cfg.name, cfg.description, cfg.prompt, cfg.category, cfg.topics, cfg.preferred_sources, cfg.blocked_sources, cfg.enabled, cfg.schedule);
      }
